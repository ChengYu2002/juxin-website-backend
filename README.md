...
- Authentication is intentionally minimal in V1:
  - Admin routes are protected via a static `ADMIN_TOKEN` (Bearer token)
  - No user system / JWT login implemented yet
- Inquiry anti-spam strategy:
  - Rate limiting + dedupe + safe email throttling
- Designed as a stable backend foundation for future admin UI and product management features.
